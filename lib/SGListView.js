...
const PrivateMethods = {
   ...
   */
  updateCellsVisibility(cellData, changedRows, rowsToKeepVisible) {
      ...
      if (changedRows.hasOwnProperty(section)) { // Good JS hygiene check
        ...
        for (const row in currentSection) {
          if (currentSection.hasOwnProperty(row)) { // Good JS hygiene check
            const mustKeepVisible = rowsToKeepVisible.indexOf(row) > -1;
            const currentCell = cellData[section][row];
            const currentCellVisibility = mustKeepVisible ? true : currentSection[row];
...
const SGListView = React.createClass({
  ...
  onChangeVisibleRows(visibleRows, changedRows) {
    ...
    PrivateMethods.updateCellsVisibility(this.cellData, changedRows, this.props.rowsToKeepVisible);
